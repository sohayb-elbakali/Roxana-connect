...
const ProfileRequired = ({ component: Component, profiles: { profile, loading }, getCurrentProfile }) => {
  ...
  useEffect(() => {
    if (!profile && !loading && !initialCheckDone) {
      getCurrentProfile();
      setInitialCheckDone(true);
    }
  }, [profile, loading, initialCheckDone, getCurrentProfile]);

  // Delay showing "create profile" message to avoid flash
  useEffect(() => {
    if (!loading && profile === null && initialCheckDone) {
      // Wait 1 second before showing create profile message
  ...
  }, [loading, profile, initialCheckDone]);

  // If we have a profile, render immediately
  if (profile) {
    return <Component />;
  }

  // Show loading spinner while checking profile
  if (loading || (!profile && !showCreateProfile)) {
    return (
  ...
  // If profile doesn't exist after delay, show create profile message
  if (profile === null && showCreateProfile) {
    return (
...
export default connect(mapStateToProps, { getCurrentProfile })(ProfileRequired);
